...
### P > 0.5
Even though the radial knob in the GUI given provides room to increase the P value to 4, anything over 0.5 caused significant instability. Random positions would cause the actuator to move almost violently and would never stabilize to a target, even with my hand applying force to the actuator. I did not play with these values for long, as I was worried about damage to the Haply.
...
## Part 5
### Set Loop-time
The lats part of the lab entails us manipulating the loop-time parameter. A lower loop-time has the effect of making the system go _faster_. This update is happening at the faster_lower loop-time. This in turn, has the effect of creating a more continuous movement, as updates are happening at quicker rate. A larger loop-time seems to add what I would call a /delay_. This often had the effect of overshooting of the target. For example, the target along the circular path would continue to move around, however the actuator would only move every so often. This had the effect of building up error in the system and an incorrect vector towards where the target used to be. I found that this could quickly create an unstable environment for the actuator, as the overshooting taking place quickly needed to be stopped manually.

In order to compensate for a larger delay, I did find that a larger Integral value as well as a smaller Derivative could help in the stability of the system.
